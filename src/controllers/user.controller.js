...
export async function cadastrarUsuario(req, res) {
  try {
    ...
    if (exists && exists.length > 0) return res.status(400).json({ erro: "E-mail já cadastrado" });
...
export async function atualizarUsuario(req, res) {
  try {
    ...
    if (Object.keys(payload).length === 0) return res.status(400).json({ erro: "Nada para atualizar" });

    // Se senha estiver presente, hash antes
    if (payload.senha) {
      payload.senha = await bcrypt.hash(payload.senha, 10);
    }

    // evitar mudar email para um já existente - verificação simples
    if (payload.email) {
      const { data: exists } = await supabase.from("usuarios").select("id").eq("email", payload.email.toLowerCase()).limit(1);
...
export async function cancelarConta(req, res) {
  try {
    ...
    const { data, error } = await supabase.from("usuarios").delete().eq("id", decoded.id);
